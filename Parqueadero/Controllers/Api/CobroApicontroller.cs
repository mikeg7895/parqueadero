...
public class CobroApicontroller : ControllerBase
{
    ...
    public async Task<ActionResult<Cobro>> GenerarCobro([FromRoute] int idReserva, [FromBody] CobroRequest request)
    {
        try
        {
            var cobro = await _cobroServicio.GenerarCobro(idReserva, request.CodigoDescuento);
            await _reciboServicio.Insertar(cobro, request.EnviarCorreo);
            return Ok(cobro);
        }
        catch (Exception ex)
        {
